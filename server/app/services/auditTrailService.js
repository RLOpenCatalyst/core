...
auditTrailService.getMonthWiseData = function getAuditTrailList(auditTrailQuery, period, callback) {
    ...
    async.waterfall([
        ...
        function (next) {
            ...
            match['botID'] = { $in: snowbotsid }
            match['user'] = auditTrailQuery.user;
            match['date'] = { $gte: sdt, $lte: edt };
                    ...
                    if (err) {
                        ...
                        for (i in data) {
                            ...
                            if (period == 'daily') {
                                ...
                                result.push({ name: item.name, count: counts })
                            }
                        }
                        botsWithCounts = result;
                        botIds = result.map((item) => { return ObjectId(item.name) })
                        if (botIds.length > 0) { next(null, []) } else { next(); }


                    }
                });
        },
        function (summaryData, next) {
            botDao.find({
                _id: {
                    $in: botIds
                }
                ...
                bots.forEach((item) => { botnames[item._id.toString()] = item.name });
                next()
            })
        },
        function (next) {
    ...
    async.waterfall([
        ...
        function (next) {
            ...
            if (auditTrailQuery.actionStatus) {
                ...
                if (status.length > 1) {
                    group = { _id: null, totalticketsresolved: { $sum: { $sum: ['$successCount', '$failedCount'] } } };
                } else if (auditTrailQuery.actionStatus == 'success') {
                    match['botID'] = { $in: snowbotsid };
                    group = { _id: null, totalticketsresolved: { $sum: '$successCount' } };
                } else if (auditTrailQuery.actionStatus == 'failed') {
                    group = { _id: null, totalticketsresolved: { $sum: '$failedCount' } };
                } else {
                    group = { _id: null, totalticketsresolved: { $sum: { $sum: ['$successCount', '$failedCount'] } } };
                }
            } else {
            ...
            aggregateQuery.push({ $group: group })
