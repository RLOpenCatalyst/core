...
module.exports.setRoutes = function (app, sessionVerification) {
    ...
    app.get('/d4dMasters/readmasterjsonnew/:id', function (req, res){
        if(req.params.id ==='undefined' || typeof req.params.id ==='undefined'){
            logger.debug('Id is pass as undefined in URL');
            res.send([]);
        }else {
            settingsService.getSettingDetailsById(req.params.id, req.session.user.cn,
                req.query.source ? req.query.source : null, req.query.orgId ? req.query.orgId : null, function (err, results) {
                    if (err) {
                        logger.error(err);
                        res.status(500).send("Failed to fetch setting Details against ID:" + req.params.id);
                        return;
                    } else {
