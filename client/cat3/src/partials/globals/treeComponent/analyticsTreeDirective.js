...
(function(angular) {
	'use strict';
	angular.module('analyticsTreeView', []).directive('analyticsTreeModel', ['$compile', function($compile) {
		return {
			restrict: 'A',
			link: function(scope, element, attrs) {
				...
				//tree model
				var treeModel1 = attrs.analyticsTreeModel;
                console.log(treeModel1);
				//node id
				...
				var template =
					...
					'<a href="#/dashboard/analytics/{{node.itemtype}}/{{node.rowid}}"><span data-ng-class="node.selected" data-nodetype="{{getNodeType(node)}}" id="{{node.rowid}}">{{node.' + nodeLabel + '}}</span></a>' +
					'<i class="caret-spacing fa fa-caret-left pull-right" data-ng-show="node.selected"></i>' +
					'<div data-ng-hide="node.collapsed" data-tree-id="' + treeId + '" data-analytics-tree-model="node.' + nodeChildren + '" data-node-id=' + nodeId + ' data-node-label=' + nodeLabel + ' data-node-children=' + nodeChildren + '></div>' +
					'</div>' +
					'</li>' +
					'</ul>';
